...
class marginal_utility_AgentHandler(AgentHandler):
    """Agent with highest marginal utility is selected in case of conflicts"""
    ...
    def _preprocess_actions(
        ...
        # 2. 计算距离
        distance = torch.norm(current_loc - target_loc, dim=-1)
        
        # 3. 获取智能体的剩余容量
        remaining_capacity = td["agents_capacity"] - td["used_capacity"]
        
        # 4. 获取目标节点的需求
        target_demand = gather_by_index(td["demand"], actions, dim=-1)
        
        # 5. 计算边际效用：考虑距离、剩余容量和需求
        # 距离越近，边际效用越高
        # 剩余容量越大，边际效用越高
        # 需求越大，边际效用越高
        utility_distance = 1.0 / (1.0 + distance)
        utility_capacity = remaining_capacity / td["agents_capacity"]
        utility_demand = target_demand / target_demand.sum(dim=-1, keepdim=True)
        
        # 综合计算边际效用
        marginal_utility = (utility_distance + utility_capacity + utility_demand) / 3.0
        
        # 6. 按照边际效用降序排序
        _, indices = torch.sort(marginal_utility, dim=-1, descending=True, stable=True)
        sorted_actions = gather_by_index(actions, indices)
...
AGENT_HANDLER_REGISTRY = {
    ...
    "pos": marginal_utility_AgentHandler,  # 修正：使用正确的类名
    "marginal_utility": marginal_utility_AgentHandler,  # 添加新的处理程序
    "none": NoHandler,
}
