...
class FAMARLDecoder(AttentionModelDecoder):
    def __init__(
        self,
        embed_dim: int = 128,
        num_heads: int = 8,
        ...
        soto_alpha: float = 0.5,  # 参数控制个体价值和社会价值的权重
        **kwargs,
    ):
        self.use_pos_token = use_pos_token
        self.soto_alpha = soto_alpha
        ...
        if use_graph_context:
            raise ValueError("FAMARL does not use graph context")

        super(FAMARLDecoder, self).__init__(
            embed_dim=embed_dim,
            num_heads=num_heads,
            env_name=env_name,
        ...
        # SOTO 框架所需的投影层
        self.individual_value_proj = nn.Linear(embed_dim, 1)
        self.social_value_proj = nn.Linear(embed_dim, 1)
        self.value_aggregation = nn.Linear(2, 1)
        self.fairness_attention = nn.MultiheadAttention(embed_dim, num_heads)
    ...
    def _compute_q(self, cached: PrecomputedCache, td: TensorDict):
        ...
        # 计算每个智能体的个体价值
        individual_values = self.individual_value_proj(step_context).squeeze(-1)  # [B, m]
        
        # 计算社会价值（考虑所有智能体的整体情况）
        social_values = self.social_value_proj(step_context).squeeze(-1)  # [B, m]
        
        # 计算公平性权重：基于智能体之间的价值差异
        # 1. 计算价值差异
        value_diff = individual_values.unsqueeze(1) - individual_values.unsqueeze(2)  # [B, m, m]
        value_diff = torch.abs(value_diff)
        
        # 2. 计算公平性分数（差异越小，公平性越高）
        fairness_scores = 1.0 / (1.0 + value_diff.mean(dim=-1))  # [B, m]
        
        # 3. 归一化公平性分数
        fairness_scores = fairness_scores / fairness_scores.sum(dim=-1, keepdim=True)
        
        # 4. 基于公平性分数计算注意力权重
        step_context_transposed = step_context.transpose(0, 1)  # [m, B, embed_dim]
        attn_output, _ = self.fairness_attention(
            step_context_transposed,
            step_context_transposed,
            step_context_transposed,
            key_padding_mask=None
        )
        attn_output = attn_output.transpose(0, 1)  # [B, m, embed_dim]
        
        # 5. 融合个体价值和社会价值
        combined_values = torch.stack([individual_values, social_values], dim=-1)  # [B, m, 2]
        value_weights = self.value_aggregation(combined_values).squeeze(-1)  # [B, m]
        value_weights = torch.softmax(value_weights, dim=-1)
        
        # 6. 应用 SOTO 框架：结合公平性和价值权重
        soto_weights = self.soto_alpha * value_weights + (1 - self.soto_alpha) * fairness_scores
        soto_weights = soto_weights.unsqueeze(-1)  # [B, m, 1]
        
        # 7. 计算最终的 glimpse_q
        soto_context = step_context * soto_weights + attn_output * (1 - soto_weights)
        glimpse_q = soto_context + graph_context_cache
...
class MatNetDecoder(FAMARLDecoder):
    def __init__(
        self,
        stage_idx: int,
        stage_cnt: int,
