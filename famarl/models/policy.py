...
from .decoder import MatNetDecoder, FAMARLDecoder
from .decoding_strategies import (
    FAMARLDecodingStrategy,
    famarl_get_decoding_strategy,
)
from .encoder import MatNetEncoder, FAMARLEncoder
...
class FAMARLPolicy(nn.Module):
    ...
    def __init__(
        ...
        super(FAMARLPolicy, self).__init__()
        ...
        # Encoder and decoder
        if encoder is None:
            log.info("Initializing default FAMARLEncoder")
            self.encoder = FAMARLEncoder(
        ...
        if decoder is None:
            log.info("Initializing default FAMARLDecoder")
            self.decoder = FAMARLDecoder(
                embed_dim=embed_dim,
                num_heads=num_heads,
    ...
    def forward(
        ...
        **decoding_kwargs,
        ...
        decode_strategy: FAMARLDecodingStrategy = famarl_get_decoding_strategy(
            decode_type,
...
class FAMARLMultiStagePolicy(nn.Module):
    ...
    def pre_forward(self, td: TensorDict, env, num_starts: int, decode_type):
        ...
        self.decode_strategy: FAMARLDecodingStrategy = famarl_get_decoding_strategy(
            decode_type,
            num_agents=n_agents,
            agent_handler=self.agent_handler,
            use_init_logp=self.use_init_logp,
